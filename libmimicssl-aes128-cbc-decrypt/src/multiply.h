static const uint32_t MULTIPLY_0[] = {
...
static const uint32_t MULTIPLY_1[] = {
    0x0, 0xd090e0b, 0x1a121c16, 0x171b121d,
    ...
    0x3e34b93, 0xeea4598, 0x19f15785, 0x14f8598e,
    ...
    0x53eddae, 0x837d3a5, 0x1f2cc1b8, 0x1225cfb3,
    ...
    0x6dd963d, 0xbd49836, 0x1ccf8a2b, 0x11c68420,
    ...
    0xa7ca147, 0x775af4c, 0x106ebd51, 0x1d67b35a,
    ...
    0x99fead4, 0x496e4df, 0x138df6c2, 0x1e84f8c9,
    ...
    0xf427ce9, 0x24b72e2, 0x155060ff, 0x18596ef4,
    0x3b6644c5, 0x366f4ace, 0x217458d3, 0x2c7d56d8,
    0xca1377a, 0x1a83971, 0x16b32b6c, 0x1bba2567,
...
static const uint32_t MULTIPLY_2[] = {
    0x0, 0x90e0b0d, 0x121c161a, 0x1b121d17,
    ...
    0x1fe3578f, 0x16ed5c82, 0xdff4195, 0x4f14a98,
    ...
    0x1ae58231, 0x13eb893c, 0x8f9942b, 0x1f79f26,
    ...
    0x506d5be, 0xc08deb3, 0x171ac3a4, 0x1e14c8a9,
    ...
    0x10e93356, 0x19e7385b, 0x2f5254c, 0xbfb2e41,
    ...
    0xf0a64d9, 0x6046fd4, 0x1d1672c3, 0x141879ce,
    ...
    0xa0cb167, 0x302ba6a, 0x1810a77d, 0x111eac70,
    ...
    0x15efe6e8, 0x1ce1ede5, 0x7f3f0f2, 0xefdfbff,
    0x79a792b4, 0x70a999b9, 0x6bbb84ae, 0x62b58fa3,
    0x5d9fbe80, 0x5491b58d, 0x4f83a89a, 0x468da397};

static const uint32_t MULTIPLY_3[] = {
    0x0, 0xe0b0d09, 0x1c161a12, 0x121d171b,
    ...
    0x3e75f8f, 0xdec5286, 0x1ff1459d, 0x11fa4894,
    ...
    0x9838680, 0x7888b89, 0x15959c92, 0x1b9e919b,
    ...
    0xa64d90f, 0x46fd406, 0x1672c31d, 0x1879ce14,
    ...
    0xcb1670a, 0x2ba6a03, 0x10a77d18, 0x1eac7011,
...
/*
    `MULTIPLY_*[]` can be generated as follows:
...
static void
printTable(const char *name, const uint32_t *table)
{
    printf("static const uint32_t %s[] = {\n", name);
    const uint32_t *p = table;
...
int main()
{
    uint32_t multiply0[256];
    uint32_t multiply1[256];
    uint32_t multiply2[256];
    uint32_t multiply3[256];
    for (uint32_t i = 0; i < 256; ++i) {
        uint8_t m09 = multiply(i, 0x09);
        uint8_t m0b = multiply(i, 0x0b);
        uint8_t m0d = multiply(i, 0x0d);
        uint8_t m0e = multiply(i, 0x0e);
        multiply0[i] = m0e | (m09 << 8) | (m0d << 16) | (m0b << 24);
        multiply1[i] = m0b | (m0e << 8) | (m09 << 16) | (m0d << 24);
        multiply2[i] = m0d | (m0b << 8) | (m0e << 16) | (m09 << 24);
        multiply3[i] = m09 | (m0d << 8) | (m0b << 16) | (m0e << 24);
    }
    printTable("MULTIPLY_0", multiply0);
    printTable("MULTIPLY_1", multiply1);
    printTable("MULTIPLY_2", multiply2);
    printTable("MULTIPLY_3", multiply3);
}
```
*/
